...
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  // 取出现行存在的回调节点【即任务task】， 默认是null 否则为上一次执行的任务
  const existingCallbackNode = root.callbackNode;

  // Check if any lanes are being starved by other work. If so, mark them as
  // expired so we know to work on those next.
  // 用于将处于"饥饿"状态的Lanes标记为过期状态。
  markStarvedLanesAsExpired(root, currentTime);

  // Determine the next lanes to work on, and their priority.
  // 基于root.pendingLanes，用于获取下一个执行任务的调度优先级
  ...
  // 获取lanes批次集合中最高优先级的lane，作为本次回调任务的优先级
  const newCallbackPriority = getHighestPriorityLane(nextLanes);

  // Check if there's an existing task. We may be able to reuse it.
  // 取出现有任务的优先级，默认是 0（初始化的时候）或者上一次任务执行的优先级
  const existingCallbackPriority = root.callbackPriority;

  // 判断新的优先级与当前任务优先级是否相等
  // 如果相同，则表示优先级无变化，可以重用当前任务
  // 这种情况一般出现在：多个状态触发的更新，可以共用一个调度任务，不会发起新的调度，或者上一个任务没有执行结束
  if (
  ...
  if (includesSyncLane(newCallbackPriority)) {
    ...
    // internal queue
    // 就根据根节点的Tag来选择调度方式 3 （react 18 默认是并发模式更新所以会直接走 else 的逻辑）
    ...
    // 注意这里会先同步的放回 scheduleCallback 执行结果，
    // 但是 scheduleCallback 内部会 调用 schedulePerformWorkUntilDeadline 创建下一个宏任务开始执行更新任务
    // 而 newCallbackNode 会被先挂载到 root.callbackNode 上，可以访问
...
// 每一个任务的实际执行，这里会 调用 ensureRootIsScheduled 形成递归调用，一直到更新任务执行完毕
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  const shouldTimeSlice =
    // 1，取反，表示不是并发执行的调度任务 （即：修改默认并发更新，或者使用useDeferredValue等并发hooks）
  ...
  if (exitStatus !== RootInProgress) {
    // 1，如果渲染出错，则会尝试重新渲染【这次会以同步渲染模式】，如果还是失败，将放弃提交生成的FiberTree （暂不考虑）
    ...
    if (exitStatus === RootDidNotComplete) {
      ...
      // # render阶段完成 进入 commit 阶段
      finishConcurrentRender(root, exitStatus, lanes);
    }
  }
  // 开启下一次任务调度，若 还有任务，会创建新的任务，若没有任务，会将 root.callbackNode = null 表示当前调度任务结束
  ensureRootIsScheduled(root, now());
  // 满足该条件是，则表明当前的调度任务没有完成，需要继续执行
  if (root.callbackNode === originalCallbackNode) {
    ...
    // 再次放回一个调度任务
    // 所以 currentTask.callback 的执行结果会得到一个函数，然后将这个有赋值给 currentTask.callback 待到下个宏任务继续执行
 ...
 * 
 * @param {*} root 
 * @description 调用renderRootSync方法创建FiberTree。然后将创建完成的FiberTree挂载到root应用根节点上。
...
function renderRootSync(root: FiberRoot, lanes: Lanes) {
  ...
  outer: do {
    try {
      // workInProgressSuspendedReason 表示 渲染过程中暂停的原因，NotSuspended 表示没有暂停
      if (
        workInProgressSuspendedReason !== NotSuspended &&
        workInProgress !== null
      ) {
